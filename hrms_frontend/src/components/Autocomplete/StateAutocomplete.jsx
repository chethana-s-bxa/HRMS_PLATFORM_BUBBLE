...
import { filterStates as filterStatesByCountry } from '../../constants/countriesData';
...
const StateAutocomplete = ({
  ...
  const handleInputChange = (e) => {
    ...
    if (text.trim()) {
      const filtered = filterStatesByCountry(country, text);
  ...
  const handleSelectSuggestion = (suggestion) => {
    ...
    onChange({
      target: { name: 'state', value: suggestion },
    });
  };
  ...
  return (
    <div className="position-relative">
      <input
        ref={inputRef}
        type="text"
        className={`form-control ${className} ${!country ? 'opacity-50' : ''}`}
        placeholder={placeholder}
        value={input}
        onChange={handleInputChange}
        onKeyDown={handleKeyDown}
        onFocus={() => country && input.trim() && suggestions.length > 0 && setShowSuggestions(true)}
        onBlur={onBlur}
